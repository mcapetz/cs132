 ...
 */
public class J2SVisitor23 extends GJNoArguDepthFirst<String> implements GJNoArguVisitor<String>{
...
//    int name_count = 0;
   ...
   public J2SVisitor23(SymbolTable symbol_table, NameGenerator name_generator, Map<String, StringPair> method_ret_label) {
      ...
      if(curr_sym != null) {
         ...
         // System.out.println("member: " + root_table.getType(curr_sym.curr_table.parent_name).member_vars);
            ...
            // System.out.println(root_table.getType(curr_sym.curr_table.parent_name).name);
               ...
               if(root_table.getType(curr_sym.curr_table.parent_name) == null) {
                  System.err.println(curr_sym.name);
                  System.err.println(curr_sym.child_table.parent_name);
                  // throw new RuntimeException("aiya");

               }
               mem_var_arr = root_table.getType(curr_sym.child_table.parent_name).member_vars;
            }
...
//    System.out.println("I GOT HERE");
      ...
      if(sym != null && sym.child_table.getParentName() != null) {
         ...
         System.err.println("here is the grandparent: " + parent_sym.child_table.getParentName());

         // let's see if there is another parent
         grandparent_sym = root_table.getType(parent_sym.child_table.getParentName());
         if(grandparent_sym != null) {
            System.err.println("there is a grandparent");

            num_bytes_to_allocate_parent += (1 + grandparent_sym.member_var_count) * 4;
            num_bytes_to_allocate_for_method_parent += grandparent_sym.num_methods * 4;
         }
      }
      if(do_inheritance) {
      ...
      //   Integer num_bytes_to_allocate_for_method = (1 + sym.member_var_count) * 4;
        ...
        if(do_inheritance) {
            if((sym.num_methods + parent_sym.num_methods + grandparent_sym.num_methods) > 0) {
     ...
     if(sym == null) {
      ...
      if(do_inheritance) {
         ...
         for(int i = 0; i < parent_sym.method_array.size(); i ++) {
         ...
         // now try grandparent
         if(grandparent_sym != null) {
            System.err.println("grandparent methods: " + grandparent_sym.method_array);
            
            Integer new_prev_size = sym.method_array.size() + parent_sym.method_array.size();
            System.err.println("new prev size: " + new_prev_size);
            for(int j = 0; j < grandparent_sym.method_array.size(); j ++) {
               System.err.println("cur met: " + grandparent_sym.method_array.get(j));
               System.err.println(sym.method_array);
               System.err.println(sym.name);
               if(!sym.method_array.contains(grandparent_sym.method_array.get(j))) {
                  System.err.println("adding met");
                  String temp_name = name_generator.generateName();
                  program_string += temp_name + " = @" + grandparent_sym.dtype + name_generator.capitalizeFirstLetter(grandparent_sym.method_array.get(j)) + "\n";
                  program_string += "[vmt_" + sym.dtype + " + " + (j + new_prev_size - 1) * 4 + "] = " + temp_name + "\n";
               }
      ...
      // now see for inheritance
      if(do_inheritance) {
         System.err.println("jeere");
         Integer prev_size1 = sym.member_var_count;
         for(int i = 0; i < parent_sym.member_var_count; i ++) {
            String name_0 = name_generator.generateName();
            program_string += name_0 + " = 0\n"; // here!!!
            program_string += "[" + name_w + " + " + 4 * (i+1 + prev_size1) + "] = " + name_0 + "\n";
            String var_name = parent_sym.member_vars.get(i);
            mem_var_map.put(var_name, name_0);
         }

         // now see for grandparent
         if(grandparent_sym != null) {
            System.err.println("grandparent mem vars");
            Integer new_prev_size = sym.member_var_count + parent_sym.member_var_count;
            for(int i = 0; i < grandparent_sym.member_var_count; i ++) {
               String name_0 = name_generator.generateName();
               program_string += name_0 + " = 0\n"; // here!!!
               program_string += "[" + name_w + " + " + 4 * (i+1 + new_prev_size) + "] = " + name_0 + "\n";
               String var_name = grandparent_sym.member_vars.get(i);
               mem_var_map.put(var_name, name_0);
            }
         }
      }
