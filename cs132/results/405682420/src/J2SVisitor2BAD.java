 ...
 */
public class J2SVisitor2BAD extends GJNoArguDepthFirst<String> implements GJNoArguVisitor<String>{
...
//    int name_count = 0;
   ...
   public J2SVisitor2BAD(SymbolTable symbol_table, NameGenerator name_generator, Map<String, StringPair> method_ret_label, Map<String, String> type_to_w) {
    //   this.program = program;
    this.name_generator = name_generator;
    this.symbol_table = symbol_table;
    this.root_table = symbol_table;
    this.method_ret_label = method_ret_label;
    this.type_to_w = type_to_w;
   ...
   public String visit(MessageSend n) { // for the message send, look at the expr list
      ...
      //   System.err.println("try: " + try_to_get_alloc_w);
        ...
        if(D == "this") {
            program_string += name_w + " = [" + D + " + 0]\n";
         }
         else if(alloc_w != null) {
            program_string += name_w + " = [" + alloc_w + " + 0]\n";
         }
         else {
      ...
      if(D == "this") {
            ...
            if(expr_v != null) {
                  program_string += ret_v + " = call " + name_w + "(" + D + " " + expr_string + ")\n"; // this is the problem
            }
            else {
                  program_string += ret_v + " = call " + name_w + "(" + D + ")\n";
            }
         }
         else if(alloc_w != null) {
            if(expr_v != null) {
                  program_string += ret_v + " = call " + name_w + "(" + alloc_w + " " + expr_string + ")\n"; // this is the problem
            }
            else {
                  program_string += ret_v + " = call " + name_w + "(" + alloc_w + ")\n";
            }

         }
         else {
            if(expr_v != null) {
                  program_string += ret_v + " = call " + name_w + "(" + sym3.allocation_var_w  + " " + expr_string + ")\n"; // this is the problem
            }
            else {
                  program_string += ret_v + " = call " + name_w + "(" + sym3.allocation_var_w  + ")\n";
            }
         }
...
alloc_happened = true;
    ...
    //   System.out.println("methods: " + sym.method_array);
      ...
      for(int i = 0; i < sym.member_var_count; i ++) {
         ...
         // System.out.println("sym: " + sym.name);
