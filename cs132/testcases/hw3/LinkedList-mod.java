...
class List{
	...
	List aux03 ;
	List aux02 ;
	aux03 = this ; // this is not working!
	// System.out.println((this.getElem()).getAge());
	aux02 = new List();
	...
	var_elem = elem ;
	// temp = e.GetAge();
	// System.out.println(temp); // this is good
	// if((!var_end) && (!ret_val)) {
	// if(true && true) {
	...
	while ((!var_end) && (!ret_val)){
		...
		if (aux04 < 0) { 
			...
			temp_e = (aux01.GetNext()).GetElem();
			temp = temp_e.GetAge();
			// System.out.println(temp);
		    aux05 = prev.SetNext(aux01.GetNext()); // here
			temp_e = ((prev.GetNext()).GetElem());
			temp = temp_e.GetAge();
			// System.out.println(temp); // should be 25
		    System.out.println(0-555);
...
class LL{
	...
	head = head.Delete(el02); // i think i have problems here
	aux01 = head.Print();
